...
router.post("/webhook/respond", async (req: Request, res: Response) => {
  let language = "en";
  ...
  try {
    ...
    const parsed = RespondPayloadSchema.safeParse(req.body);
    if (!parsed.success) {
      res.status(400).json({ error: "Invalid payload", issues: parsed.error.issues });
      return;
    }

    const { slug, senderId, message, channel } = parsed.data;
    ...
    logger.error({ err }, "Respond.io pipeline error");
    const fallback =
      language === "de"
    ...
    res.json({ reply: fallback, action: "escalate_human", intent: "out_of_scope", data: {} });
  }
});
